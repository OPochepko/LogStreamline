...
@CommandLine.Command(name = "streamline", mixinStandardHelpOptions = true, description = {"This application can be used " +
        ...
        "Aggregators:  You can aggregate filtered data by User or by time Unit (e.g. SECONDS, MINUTES and other ChronoUnit values)"})
public class Configurator implements Callable<Integer> {
    ...
    @CommandLine.Option(names = {"-at", "-aggregateTime"}, description = "Time unit to aggregate by time units(e.g. SECONDS, MINUTES and other ChronoUnit values)")
    ...
    private void formAggregator() {
        ...
        if (aggregateTimeUnit != null)
            addAggregator(new AtomicAggregator<UserDateTimeMessageFileLine, String>(v -> v.getDateTime().truncatedTo(ChronoUnit.valueOf(aggregateTimeUnit)).format(DateTimeFormatter.ISO_LOCAL_DATE_TIME)));
    }
